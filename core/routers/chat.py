                    ...
                    try:
                        ...
                        if recipient_id:
                            ...
                            try:
                                ...
                                # live push failure shouldn't break anything
                                pass
                    except Exception:
                        try:
...
async def get_chat_history(
    ...
    """Get chat history for an issue (driver endpoint). Messages are decrypted before returning."""
...
async def get_chat_status(
    ...
    """Get chat status for an issue (driver endpoint)"""
    result = await db.execute(select(Issue).where(Issue.id == issue_id))
    issue = result.scalar_one_or_none()
    ...
    if issue.status == "completed":
        ...
        message = "Chat not available. Issue not assigned yet."
    else:
...
async def websocket_info():
    ...
    **Error:**
    ...
    ```javascript
    const ws = new WebSocket('ws://localhost:8002/api/chat/ws/issue-uuid?token=your-jwt-token');
