...
async def register_driver_with_upload(
    ...
    try:
        ...
        driver = Driver(
            ...
            password=get_password_hash(password) if password else get_password_hash("temp_password"),
...
async def register_admin(
    ...
    try:
        ...
        result = await db.execute(select(Admin).where(Admin.email == encrypted_email))
        if result.scalar_one_or_none():
            raise HTTPException(status_code=400, detail="Email already registered")
...
@router.post("/api/auth/login/driver/email")
async def driver_email_login(
    request: EmailPasswordLogin,
    db: AsyncSession = Depends(get_db)
):
    """Driver login with email and password"""
    from core.utils.security import verify_password
    from core.utils.field_encryption import decrypt_email, decrypt_field, decrypt_phone
    
    try:
        logger.info(f"Login attempt for email: {request.email}")
        
        # Try encrypted email first
        encrypted_email = encrypt_email(request.email)
        logger.info(f"Encrypted email: {encrypted_email}")
        
        result = await db.execute(select(Driver).where(Driver.email == encrypted_email))
        driver = result.scalar_one_or_none()
        
        # If not found, try plain email (for non-encrypted data)
        if not driver:
            logger.info("Driver not found with encrypted email, trying plain email")
            result = await db.execute(select(Driver).where(Driver.email == request.email))
            driver = result.scalar_one_or_none()
        
        if not driver:
            logger.error(f"Driver not found for email: {request.email}")
            raise HTTPException(status_code=401, detail="Invalid email or password")
        
        logger.info(f"Driver found: {driver.id}, has password: {bool(driver.password)}")
        
        if not driver.password:
            logger.error(f"Driver {driver.id} has no password set")
            raise HTTPException(status_code=401, detail="Invalid email or password")
        
        password_valid = verify_password(request.password, driver.password)
        ...
        logger.info(f"Driver approval_status: {driver.approval_status}")
        ...
        logger.error(f"Driver email login error: {str(e)}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Login failed: {str(e)}")

# Admin Email/Password Login
@router.post("/api/auth/login/admin/email")
async def admin_email_login(
    ...
    try:
        ...
        return {
            ...
            "admin": {
                "admin_id": str(admin.id),
                "email": decrypt_email(admin.email) if admin.email else None,
                "phone_number": decrypt_phone(admin.phone_number) if admin.phone_number else None
            }
        }
        ...
        logger.error(f"Admin email login error: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Login failed: {str(e)}")
