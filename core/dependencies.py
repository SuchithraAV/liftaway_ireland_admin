...
async def get_current_user(
    ...
    elif role_value == UserRole.DRIVER.value:
...
async def get_current_driver(current_user = Depends(get_current_user)):
    ...
    if current_user.approval_status != "approved":
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Driver not approved yet"
        )
    return current_user

async def get_current_driver_any_status(current_user = Depends(get_current_user)):
    """Get current driver regardless of approval status - for document access"""
    if not isinstance(current_user, Driver):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Not authorized as driver"
