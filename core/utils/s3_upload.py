...
async def upload_file_to_s3(
    file: UploadFile,
    driver_id: str,
    file_type: str  # e.g., "license_front", "govt_id", "vehicle", "rc_book", "pollution_cert"
) -> Optional[str]:
    """
    Upload a file to UTHO Object Storage bucket under driver's folder
    
    Args:
        file: FastAPI UploadFile object
        driver_id: The driver's UUID for folder organization
        file_type: Type of document for naming
    
    Returns:
        The object storage key (e.g., drivers/uuid/license_front.jpg) or None if upload fails
    """
    ...
    try:
        ...
        # Create object storage key: drivers/driver_id/file_type.extension
        object_key = f"drivers/{driver_id}/{file_type}.{file_extension}"
        ...
        logger.error(f"Object Storage Upload Error: {e}")
        return None
...
def get_full_url(object_key: str) -> str:
    ...
    Args:
        object_key: The object storage key (e.g., drivers/uuid/license_front.jpg)
...
async def delete_file_from_s3(object_key: str) -> bool:
    ...
    try:
        ...
        logger.error(f"Object Storage Delete Error: {e}")
        return False
    except Exception as e:
